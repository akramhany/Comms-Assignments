...
def add_awgn(signal, sigma2):
    ...
    noise = np.random.normal(0, np.sqrt(sigma2), signal.shape)
    return signal + noise
...
def run_awgn_experiment(
    s1, s2, phi1, phi2, num_samples=100, eb_sigma2_db_list=[-5, 0, 10, 15]
):
    ...
    for db in eb_sigma2_db_list:
        ...
        plt.scatter(*zip(*r1_points), label="r1 (s1 + noise)", alpha=0.7, color="blue")
        plt.scatter(*zip(*r2_points), label="r2 (s2 + noise)", alpha=0.7, color="red")
        # Plotting the ideal signal space
...
if __name__ == "__main__":
    ...
    # signal one is all ones
    s1 = np.concatenate([np.ones(100)])
    # signal two is 75 ones and 25 -1
    s2 = np.concatenate([np.ones(75), -1 * np.ones(25)])
